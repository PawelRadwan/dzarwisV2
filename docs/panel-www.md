...
- **Wybieg pomp po grzaniu (automatycznie):** sterownik nie wyłącza pomp po grzaniu ([pompa-ciepla.md](pompa-ciepla.md#pułapki)), więc panel przy każdym stopie sprężarki (stan PRACA → inny) wysyła `samepompy 10 10` — wybieg jak w konfiguracji (`pco_stop`/`pko_stop`); widoczny jako licznik pomp.
