...
- **Jednorazowe akcje (`data=`) w latach 2016–2031 nie działają — błąd `kotek_rpi`.** Kotek zapisuje rok z tego zakresu o 16 za duży (sprawdzone 2026-09-23 na akcji `pompy` 0 min dla lat 2010–2045: 2010–2015 i 2032–2045 poprawnie, 2016→2032 … 2026→2042 … 2031→2047). Żaden wpis nie daje w sterowniku roku 2026. Rok dwucyfrowy albo < 2010 kotek odrzuca (`Niepoprawna data w akcji`), **ale mimo to wgrywa akcję jako `codziennie 00:00`** — po każdym błędzie trzeba wgrać pusty program S. Dlatego panel wgrywa ręczne grzanie jako akcję z dniem tygodnia (`dni="SR"`, wydruk `..S....`) i czyści S po końcu grzania. Wyczyszczenie S nie przerywa akcji, która już ruszyła (test na pompach 2026-09-23).
- **Pompy nie kończą wybiegu po grzaniu — błąd sterownika.** Po wyłączeniu sprężarki pompy CO i kolektora powinny pracować `pco_stop`/`pko_stop` = 10 min, a pracują bez końca (2026-09-23: sprężarka stop 13:23:39, odpoczynek, „Gotowa”, pompy dalej po 28 min, ~280 W; status nie ma żadnego licznika). Licznik `samepompy` kończy się poprawnie — stąd dawne obejście „pompy 5 min” na końcu programów. **W stanie ODPOCZYNEK (2 min po stopie sprężarki) `samepompy` jest ignorowane** (2026-09-24: wysłane o 14:03:22, pompy chodziły dalej); w stanie GOTOWA działa — sterownik przechodzi w stan `POMPY` („Same Pompy KOL:…”) i po czasie wyłącza pompy. Panel po każdym stopie sprężarki czeka na GOTOWA, wysyła `samepompy 10 10` i sprawdza przejście w POMPY (do 3 prób co minutę); dopiski w programach mogą zostać jako zabezpieczenie, gdy panel nie działa.
