...
- **Ręczne grzanie** (domyślnie 1 h, 0,5–8 h): panel wgrywa program S z akcją grzania w **dniu tygodnia** startu (`dni="SR"`, nie `data=` — kotek psuje rok, [pompa-ciepla.md](pompa-ciepla.md#pułapki)) na najbliższą minutę **zegara sterownika**, sprawdza zapis przez `czytajprogram S`, a po zaplanowanym końcu grzania wgrywa pusty S (inaczej grzałoby co tydzień; przy braku łączności ponawia). Wyczyszczenie S nie przerywa trwającej akcji (sprawdzone na pompach 2026-09-23); temperatura z ustawień sterownika (powrót CO 35 °C). Licznik pokazuje „Start o … — za …”, potem „Grzeje — do końca …” (czas ze sterownika). Nie ma przycisku stop — `kotek` nie ma polecenia przerywającego grzanie.
- **Same pompy obiegowe** (domyślnie 15 min, 1–120): Pompa CO / Pompa kolektora / Obie (`samepompy`), licznik odliczający, „Zatrzymaj pompy” (`samepompy 1 1`, staną po ~1 min) jest widoczny tylko, gdy pompy pracują w stanie „Gotowa” bez licznika (zawieszone po grzaniu). Pomp uruchomionych przez `samepompy` sterownik nie pozwala przerwać — panel pokazuje wtedy czas do końca ze sterownika, a API stopu zwraca błąd „staną same za … min”.
- **Wybieg pomp po grzaniu (automatycznie):** sterownik nie wyłącza pomp po grzaniu ([pompa-ciepla.md](pompa-ciepla.md#pułapki)), więc panel po każdym stopie sprężarki czeka, aż sterownik przejdzie z ODPOCZYNKU w „Gotowa” (wcześniej polecenie jest ignorowane), wysyła `samepompy 10 10` i sprawdza przejście w stan POMPY (do 3 prób co minutę) — wybieg jak w konfiguracji (`pco_stop`/`pko_stop`); widoczny jako licznik pomp.
